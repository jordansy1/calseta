...
const COLUMNS: ColumnDef[] = [
  { key: "title", initialWidth: 300, minWidth: 180 },
  ...
  { key: "close_classification", initialWidth: 200, minWidth: 140 },
  { key: "source", initialWidth: 110, minWidth: 80 },
  { key: "created_at", initialWidth: 155, minWidth: 120 },
  { key: "closed_at", initialWidth: 155, minWidth: 120 },
  { key: "occurred_at", initialWidth: 155, minWidth: 120 },
];
...
const STATUS_OPTIONS = [
  ...
  { value: "Triaging", label: "Triaging", colorClass: statusColor("Triaging") },
  {
    value: "Escalated",
    label: "Escalated",
    colorClass: statusColor("Escalated"),
  },
...
const ENRICHMENT_STATUS_OPTIONS = [
  {
    value: "Pending",
    label: "Pending",
    colorClass: enrichmentStatusColor("Pending"),
  },
  {
    value: "Enriched",
    label: "Enriched",
    colorClass: enrichmentStatusColor("Enriched"),
  },
  {
    value: "Failed",
    label: "Failed",
    colorClass: enrichmentStatusColor("Failed"),
  },
];
...
const SEVERITY_OPTIONS = [
  {
    value: "Critical",
    label: "Critical",
    colorClass: severityColor("Critical"),
  },
  ...
  { value: "Low", label: "Low", colorClass: severityColor("Low") },
  {
    value: "Informational",
    label: "Informational",
    colorClass: severityColor("Informational"),
  },
...
function classificationColor(val: string | null): string {
  if (!val) return "";
  if (val.startsWith("True Positive"))
    return "text-teal bg-teal/10 border-teal/30";
  if (val.startsWith("False Positive"))
    return "text-red-400 bg-red-400/10 border-red-400/30";
  if (val.startsWith("Benign Positive"))
    return "text-amber bg-amber/10 border-amber/30";
  return "text-dim bg-dim/10 border-dim/30";
}

export function DetectionRuleAlertsTab({
  ruleUuid,
}: {
  ruleUuid: string;
}) {
  const {
    ...
    enrichment_status: [] as string[],
  });

  const { data, isLoading, refetch, isFetching } = useAlerts({
    ...params,
    detection_rule_uuid: ruleUuid,
  });
  ...
  const uiSort = sort
    ? {
        column: reverseSortKeyMap[sort.column] ?? sort.column,
        order: sort.order,
      }
    : null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border bg-card">
        <ResizableTable storageKey="detection-rule-alerts" columns={COLUMNS}>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              ...
              <ResizableTableHead
                columnKey="close_classification"
                className="text-dim text-xs"
              >
          ...
          <TableBody>
                  ...
                  >
                    <TableCell className="truncate">
                      ...
                      {alert.tags.length > 0 && (
                        <div className="mt-1 flex gap-1">
                          {alert.tags.slice(0, 3).map((t: string) => (
                            <span
                              key={t}
                    ...
                    <TableCell>
                      <Badge
                        ...
                        className={cn(
                          "text-[11px]",
                          enrichmentStatusColor(alert.enrichment_status),
                        )}
                      >
                    ...
                    <TableCell>
                      {alert.close_classification ? (
                        <Badge
                          ...
                          className={cn(
                            "text-[11px]",
                            classificationColor(alert.close_classification),
                          )}
                        >
                          {alert.close_classification}
                        </Badge>
                      ) : (
                        <span className="text-dim text-xs">&mdash;</span>
                      )}
                    </TableCell>
                    ...
                    <TableCell className="text-xs text-dim whitespace-nowrap">
                      {alert.closed_at ? formatDate(alert.closed_at) : "\u2014"}
                    </TableCell>
                    <TableCell className="text-xs text-dim whitespace-nowrap">
                      {alert.occurred_at ? formatDate(alert.occurred_at) : "\u2014"}
                    </TableCell>
                  </TableRow>
                ))}
            {!isLoading && alerts.length === 0 && (
              <TableRow>
                <TableCell
                  colSpan={9}
                  className="text-center text-sm text-dim py-12"
                >
                  No alerts found for this detection rule
                </TableCell>
              </TableRow>
            )}
          </TableBody>
